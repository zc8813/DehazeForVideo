 ...
 */
{
    ...
    for (i = 0; i < uiSizeY; i++) {
#if 1 //have a segment fault
        pImagePointer = &pImage[uiSizeX];
        int j = 0;
        //printf("i = %d, j = %d, pImage = %x, pImagePointer = %x\n", i, j++, pImage, pImagePointer);
        while (pImage < pImagePointer) {
            //printf("i = %d, j = %d, pImage = %x, pImagePointer = %x\n", i, j++, pImage, pImagePointer);
            pulHistogram[pLookupTable[*pImage++]]++;
        }
        //printf("address %x \n", pImagePointer + uiXRes - uiSizeX);
        //pImagePointer += uiXRes;
        //printf("pImagePointer = %x\n", pImagePointer);
        //pImage = &pImagePointer[-uiSizeX];

        pImage = pImagePointer + uiXRes - uiSizeX;
        //printf("pImage = %x \n", pImage);
#else
        int j = 0;
        while(j < uiSizeX)
        {
            pulHistogram[pLookupTable[pImage[j]]]++;
            j++;
        }
