...
{
	CvCapture* cvSequence =  cvCaptureFromFile("/home/zhouc/hddisk/opencv/dehaze11/video_seq/xyj.avi");//cvCaptureFromFile(argv[1]);
	...
	cv::VideoWriter vwSequenceWriter("./result/res.avi"/*argv[2]*/, 0, 25, cv::Size(nWid, nHei), true);
	...
	dehazing dehazingImg(nWid, nHei, 16, false, false, 5.0f, 1.0f, 40);
	//dehazing dehazingImg(nWid, nHei, 16, true, false, 5.0f, 1.0f, 40);

	time_t start_t = clock();

	for( nFrame = 0; nFrame < 1/*atoi(argv[3])*/; nFrame++ )
	{
